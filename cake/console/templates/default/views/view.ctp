	...
	if (!empty($associations['belongsTo'])) {
		foreach ($associations['belongsTo'] as $alias => $details) {
			if ($field === $details['foreignKey']) {
				$isKey = true;
				echo "\t\t<div class=\"control-group\"><label class=\"control-label\"><strong><?php if (\$i % 2 == 0);?><?php __('" . Inflector::humanize(Inflector::underscore($alias)) . " : '); ?></strong></label>\n";
				echo "\t\t<div class=\"controls view-result\"><?php if (\$i++ % 2 == 0) ?>\n\t\t\t<?php echo \$this->Html->link(\${$singularVar}['{$alias}']['{$details['displayField']}'], array('controller' => '{$details['controller']}', 'action' => 'view', \${$singularVar}['{$alias}']['{$details['primaryKey']}'])); ?>\n\t\t\t&nbsp;\n\t\t</div></div>\n";
				break;
			}
		}
	}
	if ($isKey !== true) {
		echo "\t\t<div class=\"control-group\"><label class=\"control-label\"><strong><?php if (\$i % 2 == 0) ?><?php __('" . Inflector::humanize($field) . " : '); ?></strong></label>\n";
